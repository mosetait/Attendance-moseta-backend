...
exports.markLogin = async (req, res) => {
  try {
    const { employeeId, loginTime, loginLocation } = req.body;
    ...
    if (req.files) { // Assuming image is uploaded as `file` field
      const uploadedImage = await uploadImageToCloudinary(req.files.file, 'employee_logins');
      ...
      if (attendance) {
        ...
        attendance.loginImage = imageDetails;
      } else {
        ...
        attendance = new Attendance({
          ...
          loginImage: imageDetails,
          // Remove lateReason logic
        });
      }
...
exports.markLogout = async (req, res) => {
  ...
  try {
    ...
    // Upload the image to Cloudinary
    if (req.files) {
      const uploadedImage = await uploadImageToCloudinary(req.files.file, 'employee_logouts');
      const imageDetails = {
        publicId: uploadedImage.public_id,
        secureUrl: uploadedImage.secure_url
      };
      attendance.logoutImage = imageDetails;
    }
    ...
    // Determine status based on working hours or early exit
